    ...
    class StreamView extends Backbone.View
        ...
        sortActivities: (collection, updates) ->
            ...
            for key, model of collection.models
                # console.log key, collection.models.length
                if key == '0'
                    # console.log key == '0'
                    $cel = $el.find('.' + model.cid)
                    $el.prepend($cel)
                else if key == (collection.models.length-1).toString()
                    # console.log key=collection.models.length-1
                    $cel = $el.find('.' + model.cid)
                    $el.append($cel)
                else
                    # console.log key, collection.models.length-1
                    ...
                    $pel.after($cel)
                    # console.log $cel, $pel
                # console.log model.cid, key
                
        events:
            'click .activity.show--more': 'page'

window.duplicates = (array) ->
    counts = {}
    duplicates = {}
    for key, model of array
        # console.log key, model
        cur = md5(JSON.stringify(model.toJSON()))
        if !counts[cur]
            counts[cur] =
                count: 1
                id: model.toJSON()
